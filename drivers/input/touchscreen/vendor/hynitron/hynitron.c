...
static struct class *cst8xx_sprd_tpd_class;
static struct device *cst8xx_sprd_ps_cmd_dev;
#endif
...
static ssize_t cst8xx_prox_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "ps enable %d\n", cst8xx_tpd_prox_active);
}

static ssize_t cst8xx_prox_enable_store(struct device *dev, struct device_attribute *attr, const char *buf,
					size_t count)
{
	unsigned int enable;
	int ret = 0;

	hyn_info("%s real_suspend_flag=%d\n", __func__, real_suspend_flag);
...
static DEVICE_ATTR(proximity, 0664, cst8xx_prox_enable_show, cst8xx_prox_enable_store);

/* default cmd interface(refer to sensor HAL):"/sys/class/sprd-tpd/device/proximity" */
static void cst8xx_prox_cmd_path_init(void)
{
	cst8xx_sprd_tpd_class = class_create(THIS_MODULE, "sprd-tpd");
	if (IS_ERR(cst8xx_sprd_tpd_class)) {
		hyn_err("%s create sprd-tpd fail\n", __func__);
	} else {
		cst8xx_sprd_ps_cmd_dev = device_create(cst8xx_sprd_tpd_class, NULL, 0, NULL, "device");
		if (IS_ERR(cst8xx_sprd_ps_cmd_dev)) {
			hyn_err("%s create ges&ps cmd-io fail\n", __func__);
		} else {
			/* sys/class/sprd-tpd/device/proximity */
			if (device_create_file(cst8xx_sprd_ps_cmd_dev, &dev_attr_proximity) < 0) {
				hyn_err("%s create device file fail(%s)!\n", __func__, dev_attr_proximity.attr.name);
			}
		}
	}
...
{
	hyn_info("%s active=%d\n", __func__, cst8xx_tpd_prox_active);
					...
					size_t count)
{
	...
	hyn_info("%s real_suspend_flag=%d, enable=%d\n", __func__, real_suspend_flag, enable);
...
static void cst8xx_ts_resume_work(struct work_struct *work)
{
	...
	if (cst8xx_tpd_prox_active && (real_suspend_flag == 0)) {
		g_cst8xx_ts->suspended = false;
		disable_irq_wake(this_client->irq);
		hyn_info("%s: disable_irq_wake\n", __func__);
		return;
	}
	cst8xx_ts_reset();
	...
	/* if (cst8xx_tpd_prox_active &&(real_suspend_flag==0)) */
	if (cst8xx_tpd_prox_active) {
		cst8xx_prox_ctrl(cst8xx_tpd_prox_active);
		disable_irq_wake(this_client->irq);
		g_cst8xx_ts->suspended = false;
		...
		hyn_info("%s: cst8xx_tpd_prox_active = %d\n", __func__, cst8xx_tpd_prox_active);
		return;
	}
#endif
...
#if (ENABLE_DEBUG_PRINT == 1)
	hyn_info("%s resume,real_suspend_flag = %d\n", __func__, real_suspend_flag);
#endif
...
char *test_key_str[] = { "sensor_num", "screen_idac_list", "test_cp_max", "test_cp_min",
"test_cmod_max", "test_cmod_min", "fw_version"};
extern unsigned char *pcst8xx_self_test_cfg_name;
...
int hynitron_request_test_cfg(void)
{
	...
	ret = request_firmware(&fw, pcst8xx_self_test_cfg_name, g_cst8xx_ts->dev);
	cst8xx_test_cfg_len	 = 0;
	pcst8xx_test_cfg_buffer = NULL;
	if (ret == 0) {
		hyn_info("firmware request(%s) success", pcst8xx_self_test_cfg_name);
		...
		hyn_err("firmware request(%s) fail,ret=%d", pcst8xx_self_test_cfg_name, ret);
	}
