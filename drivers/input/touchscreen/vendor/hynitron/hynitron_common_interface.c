...
unsigned char cst8xx_self_test_cfg_name[][100] = {"hyn_cst836u_HuaXingDa_cfg.txt",};
unsigned char *pcst8xx_self_test_cfg_name;
...
static int cst8xx_tpd_test_cmd_show(struct tpd_classdev_t *cdev, char *buf)
{
	...
	hyn_info("tpd test resutl:%d && rawdata node failed count:%d.\n", cst8xx_tptest_result,
		   cst8xx_test_failed_count);
...
static int cst8xx_set_tp_suspend(struct tpd_classdev_t *cdev, int enable)
{
	if (enable)
		test_cst8xx_suspend();
	else
...
void cst8xx_tpd_register_fw_class(void)
{
	...
	cst8xx_test_result_node = NULL;
	pcst8xx_self_test_cfg_name = cst8xx_self_test_cfg_name[0];

	if (tpd_fw_cdev.ic_tpinfo.module_id == 0) {
		pcst8xx_self_test_cfg_name = cst8xx_self_test_cfg_name[0];
	}
